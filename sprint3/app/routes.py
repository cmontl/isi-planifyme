...
def get_weather_data(city):
    try:
        r = requests.get(f"http://localhost:5002/weather?city={city}")
...
def dashboard():
    ...
    try:
        r_movies = requests.get("http://localhost:5001/movies")
    ...
    try:
        r_events = requests.get(f"http://localhost:5003/events?city={city}")
